...
CREATE TABLE IF NOT EXISTS lib.book (
	...
	PRIMARY KEY (Book_id),
	FOREIGN KEY (Publisher_name) REFERENCES lib.publisher(Publisher_name)
);

CREATE TABLE IF NOT EXISTS lib.book_authors (
	Book_id INT NOT NULL,
	Author_name VARCHAR NOT NULL,
	PRIMARY KEY (Book_id), -- should primary key go here or use UNQIUE in Book_id?
	FOREIGN KEY (Book_id) REFERENCES lib.book(Book_id)
);
...
CREATE TABLE IF NOT EXISTS lib.book_copies (
	...
	FOREIGN KEY (Book_id) REFERENCES lib.book(Book_id),
	FOREIGN KEY (Branch_id) REFERENCES lib.library_branch(Branch_id)
);
